...
std::unique_ptr<GuiMenuList> Window_Factory::CreateMenuList(std::string_view const& windowName) const
{
	...
	else if (StrEquals(windowName, "Menu_MainCategories_NoIcons"))
	{
		returnPtr = std::make_unique<Map_Menu_MainCategories>(info.find("Menu_MainCategories_NoIcons")->second);
	}
	else if (StrEquals(windowName, "MenuListBigImages"))
	{
		returnPtr = std::make_unique<Map_Menu_InteractParty>(info.find("MenuListBigImages")->second);
	}
